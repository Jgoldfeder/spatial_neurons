...
m=__import__('timm').create_model('vit_tiny_patch16_224',pretrained=True,num_classes=100); m=m.to(dev)
@torch.no_grad()
def acc():
    m.eval();c=t=0
    for x,y in tel:
...
torch.save(m.state_dict(),__import__('os').environ.get('SN_OUT','/home/judah/spatial_neurons')+'/vittiny_cifar100_base.pt')
print('saved vittiny_cifar100_base.pt | final %.1f'%acc(),flush=True)
