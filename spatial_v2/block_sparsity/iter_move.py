...
def iterative_move(m,D,levels,F_ep,tag,net=None):
    ...
    for target in levels:
        ...
        for x,y in trl:
            ...
            with torch.no_grad():
                for l,s,msk in zip(lays,scores,masks): s.add_(-(gmat(l)*wmat(l))*msk)
...
res=iterative_move(mb,D,LEV,2,'move-'+TIL, net=(NET if TIL=='spat' else None))   # spat: spatial cost ON during finetune
