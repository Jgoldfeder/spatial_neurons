...
IMP={'mag':lambda:tp.importance.GroupHessianImportance(),
     'taylor':lambda:tp.importance.GroupHessianImportance(),
     'fpgm':lambda:tp.importance.GroupHessianImportance()}[METHOD]
t0=time.time()
...
res=[]
for step in range(8):
    if METHOD in ('taylor','hessian'):                       # Taylor needs grads accumulated before pruning
        m.zero_grad()
    ...
    res.append((100.*p/pbase,a)); print('[chan-%s] step %d | params %.1f%% dense | acc %.1f (%.0fs)'%(METHOD,step+1,100.*p/pbase,a,time.time()-t0),flush=True)
import pickle; pickle.dump(res,open('/home/judah/spatial_v2/block_sparsity/chan_hess_%s.pkl'%METHOD,'wb'))
