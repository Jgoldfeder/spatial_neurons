...
dev='cuda'; B=16; torch.manual_seed(0); np.random.seed(0)
tr=T.Compose([T.Resize(224),T.RandomHorizontalFlip(),T.ToTensor(),T.Normalize((.485,.456,.406),(.229,.224,.225))])
te=T.Compose([T.Resize(224),T.ToTensor(),T.Normalize((.485,.456,.406),(.229,.224,.225))])
full=torchvision.datasets.CIFAR100('./data',train=True,transform=tr)
idx=np.random.RandomState(0).permutation(len(full))[:12000]          # subsample -> fast
trl=DataLoader(Subset(full,idx),96,shuffle=True,num_workers=6,pin_memory=True)
...
def iterative(m,D,levels,F_ep,tag):
    ...
    return out
LEV=[40,55,65,75,80,85,90]; t0=time.time()
print('=== baseline: finetune pretrained rn18 ==='); mb=rn18(); train(mb,None,4,False); print('baseline base %.1f (%.0fs)'%(acc(mb),time.time()-t0),flush=True)
print('=== spatial: finetune with swap ==='); base=rn18(); net=swc.SpatialCNN(base,gamma=64.,device=dev,block_size=B).to(dev); train(net.model,net,4,True); print('spatial base %.1f (%.0fs)'%(acc(net.model),time.time()-t0),flush=True)
print('=== iterative prune+finetune (both) ===')
rb=iterative(mb,tiling(mb,None),LEV,1,'baseline-contig')
rs=iterative(net.model,tiling(net.model,net),LEV,1,'spatial-swap')
import pickle; pickle.dump({'baseline':rb,'spatial':rs},open('/home/judah/spatial_v2/block_sparsity/iter_prune.pkl','wb'))
