...
import sys as _sys; GAMMA=float(_sys.argv[1]); dev='cuda'; B=16; R=160; torch.manual_seed(0); np.random.seed(0)
...
mc=copy.deepcopy(mb); netc=swc.SpatialCNN(mc,gamma=GAMMA,device=dev,block_size=B).to(dev); netc.swap(block=256)
res['spatial_ft']=iterative(mc,tiling(mc,netc),LEV,2,'spatial-FT-g%g'%GAMMA,net=netc)
import pickle; pickle.dump(res,open('/home/judah/spatial_v2/block_sparsity/spatialft_g%g.pkl'%GAMMA,'wb'))
