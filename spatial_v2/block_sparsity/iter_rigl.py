...
SP=float(sys.argv[1])   # target block sparsity (fixed throughout, RigL-style)
...
# ---- RigL init: keep top (1-SP) blocks by magnitude ----
@torch.no_grad()
def block_mag():   # (list of per-block L2 over ALL blocks)
    return [torch.sqrt((Ro.t()@(wmat(l)**2)@Co)+1e-12) for l,(Ro,Co) in zip(lays,D)]
bm=block_mag(); allm=torch.cat([b.flatten() for b in bm]); thr=torch.quantile(allm,SP)
active=[(b>thr) for b in bm]                       # boolean per block: active=True
@torch.no_grad()
def enforce():
    for l,(Ro,Co),ac in zip(lays,D,active): l.weight.mul_(emask(l,(Ro@ac.float()@Co.t())))
enforce()
opt=torch.optim.Adam(m.parameters(),5e-4); res=[]; nb=16
for e in range(nb):
    # ---- RigL topology update once/epoch: drop low-mag active, grow high-grad inactive ----
    f=0.3*0.5*(1+np.cos(np.pi*e/nb))               # cosine-decay update fraction
    if e>0 and f>0.01:
        m.zero_grad()
        for i,(x,y) in enumerate(trl):
            ...
            if i>=10: break
        with torch.no_grad():
            bg=[torch.sqrt((Ro.t()@(gmat(l)**2)@Co)+1e-12) for l,(Ro,Co) in zip(lays,D)]  # block grad norm
            bmg=block_mag()
            nact=int(sum(int(a.sum()) for a in active)); k=int(f*nact)
            # drop: k active blocks with lowest weight-mag
            ...
            if k>0 and act_mags.numel()>k:
                ...
                # grow: k inactive blocks with highest grad
                ...
                for i in range(len(lays)):
                    drop=(bmg[i]<=dthr)&active[i]; grow=(bg[i]>=gthr)&(~active[i])
                    active[i]=(active[i]&~drop)|grow
        m.zero_grad(); enforce()
    m.train()
    for x,y in trl:
    ...
    a=acc(m); res.append((100.*zb/tb,a)); print('[RigL sp=%.0f] ep%2d | blk-sp %4.1f%% | acc %.1f (%.0fs)'%(SP*100,e,100.*zb/tb,a,time.time()-t0),flush=True)
import pickle; pickle.dump(res,open('/home/judah/spatial_v2/block_sparsity/rigl_%g.pkl'%SP,'wb'))
