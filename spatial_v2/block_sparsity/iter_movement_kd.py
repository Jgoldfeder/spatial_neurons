...
for e in range(nb):
    ...
    for x,y in trl:
        ...
        (F.cross_entropy(_st,y)+0.5*_kd).backward()
        with torch.no_grad():
    ...
    res.append((100.*sp,a)); print('[movement+KD lrs=%g] ep%2d | blk-sp %4.1f%% | acc %.1f (%.0fs)'%(LRS,e,100*sp,a,time.time()-t0),flush=True)
import pickle; pickle.dump(res,open('/home/judah/spatial_v2/block_sparsity/movement_kd_%g.pkl'%LRS,'wb'))
